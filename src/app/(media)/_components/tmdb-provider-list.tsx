...
export default function TmdbProviderList({
  watchProviders,
}: {
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {providers?.US?.flatrate?.map((item, index) => (
        <a
          href={providers?.US?.link}
          key={index}
          className="h-[55px] w-[55px] flex-shrink-0 transform transition-transform duration-200 hover:scale-105 hover:shadow-xl"
        >
          <Image
